...
SELECT JOB_CODE, ROUND(AVG(SALARY)), COUNT(*) -- 3
FROM EMPLOYEE -- 1
GROUP BY JOB_CODE -- 2
ORDER BY JOB_CODE; -- 4

-- EMPLOYEE 테이블에서 (FROM 절)
-- 성별(남/여)과 각 성별 별 인원수, 급여 합을 (SELECT 절)
...
GROUP BY DEPT_CODE, JOB_CODE
				   ...
				   -- ㄴ SELECT 절에 그룹함수 적용되지 않은 컬럼을 모두 GROUP BY 절에 작성되어야 함.
...
-- WHERE 절 : 각 컬럼의 값을 따져주는 것이 WHERE
-- HAVING 절 : 그룹함수로 구해 올 그룹에 대한 조건을 설정할 때 사용
...
-- ROLLUP : GROUP BY절에서 1번째 작성된 컬럼의 중간집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY -- 타입과 개수가 모두 동일해야 한다.
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
